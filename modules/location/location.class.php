...
class Location
{
    ...
    public function getArrayWard($inArrayId = array(), $districtid = 0)
    {
       ...
       // die('SELECT * FROM ' . $db_config['prefix'] . '_location_ward WHERE status=1 ' . $where . ' ORDER BY title ASC');
    ...
    public function getWardInfo($wardid, $districtid = 0)
    {
		$array_ward[] = $wardid;
        $array = $this->getArrayWard($array_ward, $districtid);
    ...
    public function locationString($provinceid = 0, $districtid = 0, $wardid = 0, $caret = ' » ', $module_url = '')
    {
        ...
        if (! empty($wardid)) {
            $ward_info = $this->getWardInfo($wardid, $districtid);
    ...
    public function buildInput($template = 'default', $module = 'location')
    {
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_data'] . '/form_input.tpl')) {
            $template = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_data'] . '/form_input.tpl')) {
            $template = $global_config['site_theme'];
        } else {
            $template = 'default';
        }

        $xtpl = new XTemplate('form_input.tpl', NV_ROOTDIR . '/themes/' . $template . '/modules/' . $site_mods[$module]['module_data']);
