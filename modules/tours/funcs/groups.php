...
if (isset($array_op[1])) {
    ...
    $stmt = $db->prepare('SELECT bid, ' . NV_LANG_DATA . '_title title, ' . NV_LANG_DATA . '_alias alias, image, ' . NV_LANG_DATA . '_description description, ' . NV_LANG_DATA . '_keywords keywords, viewtype FROM ' . $db_config['prefix'] . '_' . $module_data . '_block_cat WHERE ' . NV_LANG_DATA . '_alias= :alias');
