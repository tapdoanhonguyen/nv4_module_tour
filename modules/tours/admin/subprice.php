...
$table_name = $db_config['prefix'] . '_' . $module_data . '_subprice';
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['is_optional'] = $nv_Request->get_int('is_optional', 'post', 0);
    ...
    if (empty($error)) {
        ...
        try {
            if (empty($row['id'])) {
                $data_insert = array();
                $_sql = 'INSERT INTO ' . $table_name . ' (is_optional, weight' . $listfield . ') VALUES (:is_optional, :weight' . $listvalue . ')';
                
                $weight = $db->query('SELECT max(weight) FROM ' . $table_name . '')->fetchColumn();
                $weight = intval($weight) + 1;
                
                $data_insert['is_optional'] = $row['is_optional'];
                ...
                $stmt = $db->prepare('UPDATE ' . $table_name . ' SET ' . NV_LANG_DATA . '_title = :title, ' . NV_LANG_DATA . '_note = :note, is_optional = :is_optional WHERE id=' . $row['id']);
                $stmt->bindParam(':is_optional', $row['is_optional'], PDO::PARAM_INT);
    ...
    $lang_module['subprice_add'] = $lang_module['subprice_edit'];
...
$row['ck_is_optional'] = $row['is_optional'] ? 'checked="checked"' : '';
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        if ($view['is_optional']) {
            $xtpl->parse('main.view.loop.is_optional');
        }
...
$page_title = $lang_module['subprice'];
