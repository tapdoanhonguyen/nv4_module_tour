...
if ($nv_Request->isset_request('change_weight', 'post,get')) {
    ...
    $new_vid = $nv_Request->get_int('w', 'post', 0);
    $table_name = $db_config['prefix'] . "_" . $module_data . "_payment";
    $content = 'NO_' . $payment;
    if ($new_vid > 0) {
        $sql = 'SELECT payment FROM ' . $table_name . ' WHERE payment!=' . $db->quote($payment) . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            if ($weight == $new_vid)
                ++ $weight;
            $sql = 'UPDATE ' . $table_name . ' SET weight=' . $weight . ' WHERE payment=' . $db->quote($row['payment']);
            $db->query($sql);
        }
        $sql = 'UPDATE ' . $table_name . ' SET weight=' . $new_vid . ' WHERE payment=' . $db->quote($payment);
        $db->query($sql);
        $content = 'OK_' . $payment;
    }
...
$page_title = $lang_module['payment_setup'];
...
if (! empty($array_setting_payment) and empty($data_pay)) {
    ...
    $xtpl->assign('url_change', NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op . '&change_weight=1');
    $xtpl->assign('url_active', NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op . '&change_active=1');
...
if (! empty($data_pay)) {
    $xtpl->assign('EDITPAYMENT', sprintf($lang_module['payment_edit'], $data_pay['payment']));
