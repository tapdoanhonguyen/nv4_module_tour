...
$table_name = $db_config['prefix'] . '_' . $module_data . '_inspiration_cat';
...
if ($nv_Request->isset_request('del_block_cat', 'post')) {
    ...
    if ($bid > 0) {
        nv_insert_logs(NV_LANG_DATA, $module_name, 'log_del_inspirationcat', "block_catid " . $bid, $admin_info['userid']);
        $query = "DELETE FROM " . $table_name . " WHERE bid=" . $bid;
        if ($db->exec($query)) {
            $query = "DELETE FROM " . $db_config['prefix'] . "_" . $module_data . "_inspiration WHERE bid=" . $bid;
            $db->query($query);
            nv_fix_block_cat();
...
$xtpl = new XTemplate('inspirations.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$xtpl->assign('OP', $op);
$xtpl->assign('ROW', $row);
$xtpl->assign('ISNPIRATION_GROUPS_LIST', nv_show_inspirations_list());
