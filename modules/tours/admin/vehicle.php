...
$table_name = $db_config['prefix'] . '_' . $module_data . '_vehicle';
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        ...
        try {
            ...
            if (empty($row['id'])) {
                $data_insert = array();
                $_sql = 'INSERT INTO ' . $table_name . ' (icon, weight' . $listfield . ') VALUES (:icon, :weight' . $listvalue . ')';
                
                $weight = $db->query('SELECT max(weight) FROM ' . $table_name . '')->fetchColumn();
                $weight = intval($weight) + 1;
                
                $data_insert['icon'] = $row['icon'];
                ...
                $stmt = $db->prepare('UPDATE ' . $table_name . ' SET ' . NV_LANG_DATA . '_title = :title, ' . NV_LANG_DATA . '_note = :note, icon = :icon WHERE id=' . $row['id']);
    ...
    $lang_module['vehicle_add'] = $lang_module['vehicle_edit'];
...
$page_title = $lang_module['vehicle_manage'];
