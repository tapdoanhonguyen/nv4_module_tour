...
$sql = 'SELECT bid, ' . NV_LANG_DATA . '_title title FROM ' . $db_config['prefix'] . '_' . $module_data . '_inspiration_cat ORDER BY weight ASC';
$result = $db->query($sql);
...
if (empty($array_block)) {
    Header('Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=inspirations');
}
...
if ($nv_Request->isset_request('checkss,idcheck', 'post') and $nv_Request->get_string('checkss', 'post') == md5(session_id())) {
    $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_inspiration WHERE bid=' . $bid;
    $result = $db->query($sql);
    ...
    foreach ($id_array as $id) {
        if (! in_array($id, $_id_array_exit)) {
            try {
                $db->query('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_inspiration (bid, id, weight) VALUES (' . $bid . ', ' . $id . ', 0)');
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }
        }
    }
    nv_fix_inspiration($bid);
...
$xtpl = new XTemplate('inspiration.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
if ($listid == '' and $bid) {
    $xtpl->assign('INSPIRATION_LIST', nv_show_inspiration_list($bid));
} else {
